...
export function ShoppingItem({ item, onBought, checked, onToggleChecked, onToggleLowPrio, isLowPrio }: ShoppingItemProps) {
  ...
  return (
    <>
      <div className={`py-2.5 border-b border-gray-100 last:border-0 ${checked ? 'opacity-50' : ''}`}>
        <div className="flex items-center">
          <button
            type="button"
            onClick={() => onToggleChecked(item.id)}
            className={`flex-shrink-0 mr-3 w-6 h-6 rounded border-2 flex items-center justify-center transition-colors focus:outline-none focus:ring-2 focus:ring-blue-300 ${checked ? 'bg-blue-600 border-blue-600' : 'border-gray-300'}`}
        ...
        <div className="flex items-center justify-between mt-1 ml-9">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-full text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition-colors"
